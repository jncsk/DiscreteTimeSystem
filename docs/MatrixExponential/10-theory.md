...
> **補足：ビッグオー記法について**  
> \(\mathcal{O}(x^p)\) (ビッグオー記法) は「誤差の大きさ（オーダー）」を簡潔に表す記号で、パデ近似では「原点付近で何次まで一致し、誤差は何次から現れるか」を示している。
> 
> \[
> f(x) - R_{m,n}(x) = \mathcal{O}(x^{m+n+1})
\tag{6}
> \]は、「テイラー展開の最初の \(m+n\) 項までは完全に一致し、誤差は \(x^{m+n+1}\) の桁から現れる」という意味になる。
...
$|u| < 1$ のとき、$n \to \infty$ では $u^n \to 0$ となるため、式 (10) は以下のようになる：
