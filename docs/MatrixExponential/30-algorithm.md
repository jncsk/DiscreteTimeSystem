...
- **\(V_m\)** : 偶数次の項（係数 \(c_{2j}^{(m)}\)）
  $$
  V_m = c_0^{(m)}I + c_2^{(m)}A^2 + c_4^{(m)}A^4 + \cdots
  $$

- **\(U_m\)** : 奇数次の項（係数 \(c_{2j+1}^{(m)}\)）
  $$
  U_m = c_1^{(m)}A + c_3^{(m)}A^3 + \cdots
  $$

Padé 近似の係数 \(c_j^{(m)}\) は次式で与えられる。

$$
c_j^{(m)} = \frac{(2m-j)!\, m!}{(2m)!\, j!\, (m-j)!}, \quad j=0,1,\ldots,m
$$
...
- \(c_j^{(m)}\)：理論導出で自然に現れる係数（\(c_0=1\) の正規化）  
- \(b_j\)：\(\frac{(2m)!}{m!}\) を掛けて実装上扱いやすくした係数（整数化）  
- どちらの表現でも **同じ近似**（分子・分母を同じ定数倍しているだけ）
