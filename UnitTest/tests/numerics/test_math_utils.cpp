...
TEST(CoreMathUtils_Factorial, ReturnsSuccessForSmallN) {
    ...
    EXPECT_EQ(math_utils_factorial(0u, &out), CORE_ERROR_SUCCESS);
    EXPECT_EQ(out, 1u);

    out = 0;
    EXPECT_EQ(math_utils_factorial(1u, &out), CORE_ERROR_SUCCESS);
    EXPECT_EQ(out, 1u);

    out = 0;
    EXPECT_EQ(math_utils_factorial(5u, &out), CORE_ERROR_SUCCESS);
    EXPECT_EQ(out, 120u);
}

TEST(CoreMathUtils_Factorial, ReturnsSuccessFor20) {
    uint64_t out = 0;
    EXPECT_EQ(math_utils_factorial(20u, &out), CORE_ERROR_SUCCESS);
...
TEST(CoreMathUtils_Factorial, ReturnsInvalidArgWhenTooLarge) {
    ...
    EXPECT_EQ(math_utils_factorial(21u, &out), CORE_ERROR_INVALID_ARG);
...
TEST(CoreMathUtils_Factorial, ReturnsNullWhenResultIsNull) {
    EXPECT_EQ(math_utils_factorial(0u, nullptr), CORE_ERROR_NULL);
}
