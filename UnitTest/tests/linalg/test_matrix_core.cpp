...
TEST(MatrixCore_Create, ReturnsMATRIX_CORE_SUCCESS) {
    ...
    EXPECT_DOUBLE_EQ(matrix_ops_set(m, 1, 2, 42.0), MATRIX_CORE_SUCCESS);

    MatrixCoreStatus e2 = MATRIX_CORE_SUCCESS;
    double v = matrix_ops_get(m, 1, 2, &e2);
    EXPECT_EQ(e2, MATRIX_CORE_SUCCESS);
    EXPECT_EQ(v, 42.0);
    
    EXPECT_EQ(matrix_core_free(m), MATRIX_CORE_SUCCESS);
}

TEST(MatrixCore_Create, ReturnsMATRIX_CORE_ERR_OUT_OF_BOUNDS) {
    MatrixCoreStatus err = MATRIX_CORE_SUCCESS;
...
TEST(MatrixCore_CreateSquare, ReturnsMATRIX_CORE_SUCCESS) {
    MatrixCoreStatus err = MATRIX_CORE_SUCCESS;
...
TEST(MatrixCore_CreateSquare, ReturnsMATRIX_CORE_ERR_OUT_OF_BOUNDS) {
    MatrixCoreStatus err = MATRIX_CORE_SUCCESS;
...
TEST(MatrixCore_Free, ReturnsMATRIX_CORE_SUCCESS) {
    MatrixCoreStatus err = MATRIX_CORE_SUCCESS;
...
TEST(MatrixCore_Free, ReturnsMATRIX_CORE_ERR_NULL) {
    MatrixCoreStatus err = MATRIX_CORE_SUCCESS;
    Matrix* m = NULL;

    EXPECT_EQ(matrix_core_free(m), MATRIX_CORE_ERR_NULL);
}
